...
class InterfazUsuario:
    def __init__(self, root):
        ...
        # Botón para validar
        self.btn_validar = tk.Button(root, text="Validar", command=self.validar_expresion, bg="#28A745", fg="white")
        self.btn_validar.grid(row=2, column=0, padx=5, pady=10)

        # Botón para generar autómata
        self.btn_generar_automata = tk.Button(root, text="Generar Autómata", command=self.mostrar_automata, bg="#0078D7", fg="white")
        self.btn_generar_automata.grid(row=2, column=1, padx=5, pady=10)
    ...
    def mostrar_automata(self):
        ...
        try:
            afnd = generador.construir_grafo_desde_tokens(tokens)
